...
const GroupCheckboxList = ({ localColumns, className, draggable, checkable }) => {
  ...
  return (
    ...
    >
      <CheckboxList title="固定在左侧" list={leftList} draggable={draggable} checkable={checkable} className={className} />
      {/* 如果没有任何固定，不需要显示title */}
      <CheckboxList
        list={list}
        draggable={draggable}
        ...
        className={className}
      />
      <CheckboxList title="固定在右侧" list={rightList} draggable={draggable} checkable={checkable} className={className} />
    </div>
  );
};
